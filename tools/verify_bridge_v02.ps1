...
.EXAMPLE
.\tools\verify_bridge_v02.ps1
.EXAMPLE
.\tools\verify_bridge_v02.ps1 -ProgramRom1 C:\roms\game1.bin -ProgramRom2 C:\roms\game2.bin -SoundRom1 C:\roms\sound.bin
.EXAMPLE
.\tools\verify_bridge_v02.ps1 -ProgramRom1 C:\roms\game1.bin -ProgramRom2 C:\roms\game2.bin -ProgramRom3 C:\roms\game3.bin -ProgramRom4 C:\roms\game4.bin
#>
[CmdletBinding()]
param(
    [string]$ProgramRom1,
...
foreach ($configuration in @('Debug','Release')) {
    ...
    if ($runtimeArgs.Count -gt 0) {
        ...
        & $diagnostic --v2 @runtimeArgs
        if ($LASTEXITCODE -ne 0) { throw "$configuration v2 runtime diagnostic failed" }
    }
}

Write-Host 'Amber Bridge v0.2 verification succeeded for Debug and Release x64.'
