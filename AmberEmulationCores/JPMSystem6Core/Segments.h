...
private:
	...
	INT32 State[NUMSEGMENTS];				//Dimming State
	INT32 SavedState[NUMSEGMENTS];		//Saved Dimming State
	INT32 LastState[NUMSEGMENTS];			//Last Dimming State
	...
	INT32 MaxTimeOn[NUMSEGMENTS];			//Max Time On
	INT32 MaxTimeOff[NUMSEGMENTS];		//Max Time Off

	//Options
	INT32 FadeTime[NUMSEGMENTS];			//Fade Time
	INT32 GlowTime[NUMSEGMENTS];			//Glow Time	
	INT32 FadeTimeStore[NUMSEGMENTS];		//Fade Time
	INT32 GlowTimeStore[NUMSEGMENTS];		//Glow Time	
	UINT8 MinDimLevel[NUMSEGMENTS];		//Minium Dimming Level

	//Super Bright Stuff
	INT64 MuxStart;				//Mux Start Value
	INT64 MuxEnd;					//Mux End Value
	INT32 SuperBrightMux;			//Super Bright Mux Value
	INT64 MuxGapStat;				//Mux Gap State
	INT64 MuxGapCount[NUMSTROBELINES];		//Mux Gap Count
	INT32 SuperBrightEnable[NUMSTROBELINES];	//Super Bright Enable
	INT64 SuperBrightLevel;		//Super Bright Level

	//General Stuff	
	INT32 SegColumnData[NUMSTROBELINES];		//Lamp Column Data
	INT32 LastSegColumnData[NUMSTROBELINES];	//Last Lamp Column Data
	INT32 TimeSinceDataChange;	//Time Since Data Change
	INT32 MuxDrive;
	UINT8 Toggle;
...
public:	
	...
	INT32 GetSegState(UINT16 Num, INT32 OnCycles, INT32 OffCycles); //Lamp Brightness Calculation Sub
	void Update(void);
