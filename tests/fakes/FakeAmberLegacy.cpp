...
LEGACY_EXPORT uint8_t Initialise() {
  initialised = true;
  return 1;
}
LEGACY_EXPORT uint8_t Shutdown() {
...
LEGACY_EXPORT uint32_t LoadROM(uint8_t *a, uint8_t *b, uint8_t *, uint8_t *) {
  return initialised && a && b ? 2u : 0u;
}
LEGACY_EXPORT uint32_t LoadSoundROM(uint8_t *a, uint8_t *, uint8_t *,
                                    uint8_t *) {
