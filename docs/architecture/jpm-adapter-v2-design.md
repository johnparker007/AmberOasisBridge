...
The native reset path reinitialises runtime and defaults, so v2 configuration is adapter-owned and must be re-applied after every successful `Reset`. This supports the established `Initialise; Reset; Configure; Run` ordering. A single validated aggregate is atomic from the v2 caller's perspective; apply-mask omissions preserve defaults/current adapter settings.
...
V2 format is 48000/2/PCM_S16/interleaved. The destination must hold `frame_capacity * channels` `int16_t` values and have normal `int16_t` alignment. Capacity zero succeeds with zero written and permits a null sample pointer; otherwise null is invalid. Partial fills are permitted contractually and successful; current System 6 normally returns the full capacity. Format is available after initialisation through the instance API; frame copying is valid after initialisation and after reset/run, and consumes generated frames.

## Former snapshot discrepancies

The PA2 snapshot is packed to 4 bytes and contains broad cross-core categories. System 6 populates matrix lamps, a 512-element LED/segment plane, reels, alpha, 40 LED displays, coin mech, meters, tubes, DIPs and hoppers, while leaving many counts zero. V2 does not copy it blindly: it uses natural ABI alignment, fixed integers/Q16.16, only AB2 visual categories, and corrects the duplicated two-alpha-display claim. The PA2 seven-segment brightness expression indexes `display` rather than the display's `display*16` segment; the v2 implementation must deliberately derive each display brightness from its corresponding segment group and document the chosen representative (maximum is recommended). This choice is an open implementation-review item.
