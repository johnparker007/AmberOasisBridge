...
void FilamentLamp::SaveState(LoadSaveClass* LSC)
{
    // Temperature is the authoritative physical state.  Resistance, electrical
    // power, brightness, and colour are derived/cache values and are rebuilt on
    // load from the active bulb parameters.
    LSC->SaveToBuffer(TemperatureK);
}

void FilamentLamp::LoadState(LoadSaveClass* LSC)
{
    LSC->LoadFromBuffer(TemperatureK);

    TemperatureK = FilamentClamp(TemperatureK, Params.AmbientK, Params.MaxTemperatureK);
