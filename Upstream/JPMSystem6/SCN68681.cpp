...
void DuartScn68681::reset(LoadSaveClass * LSCIn)
{
	...
	recva = recvb = false;
	transmitA = transmitB = false;
	delayA = delayB = false;
	rtsa = rtsb = 0;
	buffposA = buffposB = 0;
	send_countA = send_countB = 0;
	send_posA = send_posB = 0;
	send_delayA = send_delayB = 0;
	lenA = lenB = 0;
	ZeroMemory(bufferA, sizeof(bufferA));
	ZeroMemory(bufferB, sizeof(bufferB));
	ZeroMemory(send_buffA, sizeof(send_buffA));
	ZeroMemory(send_buffB, sizeof(send_buffB));
}

UINT8 __fastcall DuartScn68681::GetOutputPort(void) const
{
    return op;
}
...
UINT8 * DuartScn68681::GetEDCString(void){
    ...
    return EDC.getEDCString();

}

UINT8 DuartScn68681::GetEDCAvailable(void)
{
    return EDC.GetAvailable();
}
...
bool check = true;
  ...
  if ( txa ) {
    ...
    if ( (sra & TxRDY) == 0 && TxA ) {
      ...
      buffposA = static_cast<UINT8>((buffposA + 1) & (BUFFER_SIZE - 1));
      txa = 100;  //100
      sra |= TxRDY;
      isr |= TxRDYA;
    }
  ...
  if ( txb ) {
    ...
    if ( (srb & TxRDY) == 0 && TxB ) {
      ...
      buffposB = static_cast<UINT8>((buffposB + 1) & (BUFFER_SIZE - 1));
      txb = 100;
      srb |= TxRDY;
      isr |= TxRDYB;
    }
  ...
  switch ( offset ) {
    ...
    case 2:
      ...
      switch ( (value & 0x70) >> 4 ) {
        ...
        case 3:
          ...
          break;
        case 4:
          sra &= ~(OVR + PE + FE + RB); // Reset Error Status
          break;
        case 5:
          isr &= ~DBA;
          break;
    ...
    case 3:
      ...
      if ( (mr2a & 0x80) == 0x80 ) { // Local Loopback
		...
		UINT8 EdcResponse = EDC.Write(static_cast<UINT8>(value));
        if (EdcResponse && RxA) {
            ReceiveCharA(EdcResponse);
        }
    ...
    case 0x0a:
      ...
      switch ( (value & 0x70) >> 4 ) {
        ...
        case 3:
          ...
          break;
        case 4:
          srb &= ~(OVR + PE + FE + RB); // Reset Error Status
          break;
        case 5:
          isr &= ~DBB;
          break;
