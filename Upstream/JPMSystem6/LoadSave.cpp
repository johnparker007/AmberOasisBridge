...
namespace
{
    ...
    {
        if (bytes == 0) bytes = 1;
        return new (std::nothrow) UINT8[bytes];
    }
...
bool LoadSaveClass::EnsureSaveCapacity(size_t bytesToAdd)
{
    ...
    UINT8* newBuffer = AllocBytes(newCapacity);
    if (!newBuffer) return false;

    if (savePointer > 0) {
        memcpy(newBuffer, saveBuffer, savePointer);
    }
...
bool LoadSaveClass::LoadVersionFromBuffer()
{
    UINT32 val = 0;
    if (!CanLoadBytes(4)) {
    ...
    LoadFromBuffer(val);
    LoadedFormatVersion = static_cast<INT32>(val);
    return LoadedFormatVersion == FORMATVERSION;
}

bool LoadSaveClass::IsLoadValid() const
{
    return loadBuffer != NULL && loadSize > 0 && !loadFailed && loadPointer <= loadSize;
}

INT32 LoadSaveClass::GetLoadedFormatVersion() const
{
...
// LOADS
bool LoadSaveClass::LoadInit(UINT8* FileString)
{
    loadPointer = 0;
    loadSize = 0;
    loadFailed = false;
